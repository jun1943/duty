...
$(function () {
	...
	$('#dtVehicle').datagrid({
		...
		queryParams: { 'vehicle_Query': JSON.stringify(m_Vehicle_Query)},
		pagination: true,
		fitColumns:true,
        pageNumber: 1,
        pageSize: 10, 
		...
		columns: [[
	               ...
	               { title: '车辆类型', field: 'typeName', align: 'center', width: 100 },
	               { title: '车牌号码', field: 'number', align: 'center', width: 100 },
	               { title: '车辆用途', field: 'purpose', align: 'center', width: 100 },
	               { title: '车辆品牌', field: 'brand', align: 'center', width: 150 },
	               { title: '座位数', field: 'siteQty', align: 'center', width: 150 },
	               { title: 'GPS设备ID', field: 'gpsId', align: 'center', width: 80 },
	               { title: 'GPS名称', field: 'gpsName', align: 'center', width: 80 },
	               { title: '组呼号', field: 'intercomGroup', align: 'center', width: 200 }
	        ]]
	});
...
function btnSearchAction(){
	pack_Vehicle_Query();
	$('#dtVehicle').datagrid("reload",{ 'vehicle_Query': JSON.stringify(m_Vehicle_Query)});
	$("#isSubOrg").val(0);
	$("#txtsearchnumber").val(""); 
};
function InitData(){
...
function btnDelVehicle(){
     ...
     var id = rows[0].id;
	$.messager.confirm("系统提示", "确认删除车牌号为    " + number + " 的数据信息吗？", function (r) {
        if (r) {
            deleteVehicle(id);
        }
    });
