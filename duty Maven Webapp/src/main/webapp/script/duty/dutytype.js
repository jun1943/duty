...
$(function() {
	...
	$('#dtDutyType').treegrid({
		...
		columns : [ [ {
			...
			align : 'left',
			width : 50,
			formatter : getMaxPolice
		}, {
			title : '统计显示方式',
			field : 'isShowname',
			align : 'left',
			width : 50,
			formatter : getDutyTypeDisplayType
		}, {
			title : '属性',
			field : 'properties',
			align : 'left',
			width : 150,
			formatter : getDutyTypeProperties
		}, {
			title : '关联任务',
			field : 'assoTaskType',
			align : 'left',
			width : 60,
			formatter : getDutyTypeTaskType
		}, {
			title : '着装',
			field : "attireType",
			align : 'left',
			width : 60,
			formatter : getDutyTypeAttireType
		}, {
			title : '武器',
			field : "armamentType",
			align : 'left',
			width : 60,
			formatter : getDutyTypeArmamentType
		}, {
			title : '停用',
			field : "isUsed",
			align : 'left',
			width : 30,
			formatter : getDutyTypeUsed
		}, {
			title : 'parentId',
			field : "parentId",
			align : 'left',
...
function addChildDutyType() {
	var row = $("#dtDutyType").treegrid("getSelected");
	if (row != null) {
		if (!row.isUsed) {
...
function editDutyType() {
	...
	if (row != null) {
		...
		$("input[name='attireType'][value=" + row.attireType + "]").attr(
				"checked", true); // 根据Value值设置Radio为选中状态
		$("input[name='armamentType'][value=" + row.armamentType + "]").attr(
				"checked", true); // 根据Value值设置Radio为选中状态
...
function delDutyType() {
	...
	var row = $("#dtDutyType").treegrid("getSelected");
	if (row != null) {
		var snode = $("#dtDutyType").treegrid("getChildren", row.parentId);
		if (snode.length == 1) {
			isnode = 1;
		}
		if (!row.isLeaf) {
			...
			$.messager.confirm('操作提示', "确定要删除[" + row.name + "]吗?",
					function(r) {
						if (r)
							$.ajax({
...
function unUseDutyType() {
	...
	if (row != null) {
		...
		if (!row.isLeaf) {
			$.messager.confirm('操作提示', "停用[" + row.name + "]及下属所有勤务类型?",
					function(r) {
						if (r)
							updateUsedState(row.id, !row.isUsed);
					});
